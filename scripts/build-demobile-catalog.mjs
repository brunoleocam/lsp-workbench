#!/usr/bin/env node
/**
 * PDR-007 — gera catálogo local (gitignored) a partir de docs/banco-senior.
 * Não publica dados: o JSON fica só na máquina.
 *
 * Uso:
 *   node scripts/build-demobile-catalog.mjs
 *   node scripts/build-demobile-catalog.mjs --out docs/banco-senior/.generated/catalog.json
 */
...
if (!fs.existsSync(bancoRoot)) {
  console.error("docs/banco-senior/ não encontrado (conteúdo Demóbile privado).");
  process.exit(2);
}
...
// 1) tabelas-principais.md — linhas de tabela markdown
...
// 2) dicionario-dados/tabelas/*.md — título `# CODE – Desc`
